...
$oAddons = rex_addon::get('nv_modulepreview');
if ($oAddons->getConfig("overwrite_module_select")) {
    $aModules = array();
    ...
    if ($oAddons->getConfig("show_only_gridblock")) {
        $aModules = array();
        foreach ($this->items as $aItem) {
            $iId = $aItem["id"];
    ...
    $select = nvModulepreview::getPreview($aModules, $aParams);
} else {
